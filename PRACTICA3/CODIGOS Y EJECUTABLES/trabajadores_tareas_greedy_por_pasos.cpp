...
Compilacion: g++ trabajadores_tareas_greedy_por_pasos.cpp -o ejecutable
...
int AsignarTareasATrabajadores(int **matriz, int trabajadores, int tareas, int *&tareas_asignadas, int mayor_fila_columna){
	...
	for(int i = 0; i < mayor_fila_columna; i++){   //Se mueve por los trabajadores (filas)
		...
		for(int j = 0; j < mayor_fila_columna; j++){  //Se mueve por las tareas (columnas)
			...
			//***************************************************
			bool hago = true;
			for(int t = 0 ; t < mayor_fila_columna ; t++){
				
				if(j == tareas_asignadas[t]){
			...
			if(hago){
				...
				cout<<"v"<<endl;
					...
					if(i==0){
						for(int n = 0 ; n < mayor_fila_columna ; n++){
							for(int m = 0 ; m < mayor_fila_columna ; m++){
						...
						for(int n = 0 ; n < mayor_fila_columna ; n++){
							...
							if(tareas_asignadas[n] != -1 && n!=i){
								
								for(int m = 0 ; m < mayor_fila_columna ; m++){
									
									if(m == tareas_asignadas[n]){
								...
								for(int m = 0; m< mayor_fila_columna ; m++){
										...
										for(int k = 0 ; k < i &&pintar ; k++){
											
											if(m==tareas_asignadas[k] ){
			...
			if(!ignorar){ //Si la tarea no ha sido asignada, se comprueba su coste y se compara con el coste de la tarea que por ahora iba a ser asignada 
				...
				if(matriz[i][j] < menor){ //Si el coste de esa tarea con ese trabajador es menor que la que ibamos a asignar, esta pasa a ser la asignada
					...
					cout <<"\nAsignamos la tarea " << j+1 << " al trabajador " <<i+1<<endl;
		...
		cout<<"-------------------------------------------";
		cout<<"\nFinalmente el trabajador "<< i+1 << " se ha quedado con la tarea "<< tareas_asignadas[i]+1 << " y esta desaparece de las disponibles" ;
...
int AsignarTareasATrabajadoresConSuma(int **matriz, int trabajadores, int tareas, int *&tareas_asignadas_suma, int mayor_fila_columna){
	...
	for(int i = 0; i < mayor_fila_columna; i++){   //Se mueve por los trabajadores (filas)
				...
				cout<<"v"<<endl;
					...
					if(i==0){
						...
						for(int n = 0 ; n < mayor_fila_columna ; n++){
							...
							if(tareas_asignadas_suma[n] != -1 && n!=i){
								
								for(int m = 0 ; m < mayor_fila_columna ; m++){
									
									if(m == tareas_asignadas_suma[n]){
		...
		cout<<"-------------------------------------------";
		cout<<"\nFinalmente el trabajador "<< i+1 << " se ha quedado con la tarea "<< tareas_asignadas_suma[i]+1 << " y esta desaparece de las disponibles" ;
...
int AsignarTrabajadoresATareas(int **matriz, int trabajadores, int tareas, int *&trabajadores_asignados, int mayor_fila_columna){
	...
	for(int i = 0; i < mayor_fila_columna; i++){   	//Se mueve por las tareas (columnas)
		...
		for(int j = 0; j < mayor_fila_columna; j++){ //Se mueve por los trabajadores (filas)
							...
							for(int  m = 0; m < mayor_fila_columna ; m++){
								...
								if(m<i){
									...
									for(int k = 0 ; k < i ; k++){
										...
										if(n == trabajadores_asignados[k] && m ==k)
											pint = true;
										
									}
									
									if(pint) cout<<matriz[n][m]<<" ";
									else cout<<"  ";
			...
			if(!ignorar){  //Si el trabajador aun no tiene tarea asignada, se comprueba el coste de asignarle la actual (i) y se compara con el coste que producia asignar esa tarea 
				...
				if(matriz[j][i] < menor){  //Se cumple si el coste con este trabajador es menor que el coste con el trabajador anterior al que se le ha asignado temporalmente 
					...
					cout <<"\nAsignamos el trabajador " << j+1 << " a la tarea " <<i+1;
				}
		...
		cout<<"-------------------------------------------";
		cout<<"\nFinalmente el trabajador "<< trabajadores_asignados[i]+1 << " se ha quedado con la tarea "<< i+1 << " y esta desaparece de las disponibles"<<endl; ;
	...
	for(int i = 0; i < mayor_fila_columna; i++){   	//Se mueve por las tareas (columnas)
		...
		for(int j = 0; j < mayor_fila_columna; j++){ //Se mueve por los trabajadores (filas)
			...
			for(int t = 0 ; t < mayor_fila_columna ; t++){
				...
				if(j == trabajadores_asignados_suma[t]){
							...
							for(int  m = 0; m < mayor_fila_columna ; m++){
								...
								if(m<i){
									...
									for(int k = 0 ; k < i ; k++){
										...
										if(n == trabajadores_asignados_suma[k] && m ==k)
											pint = true;
									...
									for(int k = 0 ; k < i ; k++){
										...
										if(n == trabajadores_asignados_suma[k]){
		...
		cout<<"-------------------------------------------";
		cout<<"\nFinalmente el trabajador "<< trabajadores_asignados_suma[i]+1 << " se ha quedado con la tarea "<< i+1 << " y esta desaparece de las disponibles"<<endl; ;
...
void asignacion(int **matriz, int trabajadores, int tareas, int *&tareas_asignadas, int *&trabajadores_asignados, int *&tareas_asignadas_suma, int *&trabajadores_asignados_suma, int &coste_asignar_tareas, int &coste_asignar_trabajadores, int mayor_fila_columna, int &coste_asignar_tareas_con_suma, int &coste_asignar_trabajadores_con_suma){
	...
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"                                         ASIGNANDO TAREAS A LOS TRABAJADORES"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
    ...
    cout<<"\n\n\n**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"                                       AHORA ASIGNANDO TRABAJADORES A LAS TAREAS"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
    ...
    cout<<"\n\n\n**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"                                     AHORA ASIGNANDO TAREAS A LOS TRABAJADORES CONTANDO CON LA SUMA"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	...
	cout<<"\n\n\n**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"                                       AHORA ASIGNANDO TRABAJADORES A LAS TAREAS CONTANDO CON LA SUMA"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
	cout<<"**********************************************************************************************************************"<<endl;
