...
*/
pair<int, vector<int> > DistanciaYRecorridoMinimo(int nodo_inicio, vector<int> nodos_por_pasar, vector<Ciudad>&ciudadesRecogidas){ 
	...
	if(nodos_por_pasar.size() == 0){   //CasoBase (los indices por los que hay que pasar son 0)
			...
			comprobaciones.push_back(ciudadesRecogidas[0]);                  //Introduzco la ciudad 1 que es el retorno
		...
		for(int j=0; j<nodos_por_pasar.size(); j++){
			...
			aux = DistanciaYRecorridoMinimo(elementoBorrado, nodos_por_pasar, ciudadesRecogidas);
...
void CalcularRestoDeNodos(int dimension, int ciudadInicio, vector<int> &vectorNodosPaso){
	...
	for(int i=0; i<dimension; i++){
		...
		if(i!=ciudadInicio)        //Si la ciudad es la misma de la que partimos, no la consideramos como una de las que hay que recorrer
...
void ResultadoFinal(pair<int,vector<int> > solucion, int ciudadInicio){
	...
	for(int i=solucion.second.size()-1; i>=0; i--)       //Se muestra al reves porque la recurrencia almacena los nodos de detino a origen, y el camino tiene el sentido contrario
		cout << solucion.second[i]+1 << " -> " ;

	cout<< ciudadInicio+1 << "\n";	       //Se aniade al recorrido final que se muestra por pantalla la vuelta desde la ultima ciudad a aquella de la que partio
...
int main(int argc, char * argv[]){
	...
	tiempo1 = clock(); 				//Se calcula el tiempo antes de ejecutar la funcion
	solucion = DistanciaYRecorridoMinimo( ciudadInicio, vectorNodosPaso, ciudadesRecogidas); 
	tiempo2 = clock(); 				//Se calcula el tiempo antes de ejecutar la funcion

	//Esto muestra la diferencia de tiempo entre el final y el inicial de la ejecucion del algoritmo 
	cout << dimension << "  " << ((double)(tiempo2 - tiempo1)) / CLOCKS_PER_SEC << endl;
