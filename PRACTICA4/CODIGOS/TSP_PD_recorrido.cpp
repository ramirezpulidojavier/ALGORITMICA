...
* @param ciudadInicio variable que contiene la ciudad de la que se inicia el recorrido
*/
void ResultadoFinal(pair<int,vector<int> > solucion, int ciudadInicio, vector<Ciudad> ciudadesRecogidas){
	...
	for(int i=solucion.second.size()-1; i>=0; i--)         //Muestra el recorrido al reves porque la recursividad hace que los nodos se almacenen de destino a origen, al reves
		cout << ciudadesRecogidas[solucion.second[i]].indice << " " << ciudadesRecogidas[solucion.second[i]].coordx << " " << ciudadesRecogidas[solucion.second[i]].coordy <<endl ;
...
int main(int argc, char * argv[]){
	...
	ResultadoFinal(solucion, ciudadInicio, ciudadesRecogidas);
